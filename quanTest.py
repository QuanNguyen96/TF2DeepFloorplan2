...
# def find_consecutive_ranges(points):
#     # Bỏ trùng điểm
#     unique_points = list(set(points))

#     group_by_i = defaultdict(list)
#     group_by_j = defaultdict(list)

#     for i, j in unique_points:
#         group_by_i[i].append(j)
#         group_by_j[j].append(i)

#     result = []
#     seen = set()

#     # 1. Theo hàng (i), nối các cột (j)
#     for i, js in group_by_i.items():
#         js_sorted = sorted(js)
#         for j_start, j_end in merge_consecutive(js_sorted):
#             item = {"start": (i, j_start), "end": (i, j_end)}
#             key = (i, j_start, i, j_end)
#             if key not in seen and (j_start != j_end):
#                 result.append(item)
#                 seen.add(key)
#             elif j_start == j_end:
#                 # Cho phép đoạn đơn vị đúng theo yêu cầu nếu chưa tồn tại
#                 if key not in seen:
#                     result.append(item)
#                     seen.add(key)

#     # 2. Theo cột (j), nối các hàng (i)
#     for j, is_ in group_by_j.items():
#         is_sorted = sorted(is_)
#         for i_start, i_end in merge_consecutive(is_sorted):
#             key = (i_start, j, i_end, j)
#             item = {"start": (i_start, j), "end": (i_end, j)}
#             if key not in seen and (i_start != i_end):
#                 result.append(item)
#                 seen.add(key)
...
# points = [(1,0),(1,1),(1,4),(1,5),(1,6),(2,4),(3,4)]*2  # tạo data lớn

# result_py = find_consecutive_ranges(points)
...
# def save_to_json(arr, filename):
#     # Ép kiểu về int để JSON dễ đọc, rồi convert sang list
#     arr_list = arr.squeeze().astype(int).tolist()
#     with open(filename, "w") as f:
#         json.dump(arr_list, f, indent=2)  # indent=2 để format đẹp
#     print(f"✅ Đã lưu {filename}")

# save_to_json(newr, "newr.json")
# save_to_json(newcw, "newcw.json")
...
# Giả sử newcw_color có shape (H, W, 3)
white = np.array([255, 255, 255])
mask_white = np.all(newcw_color == white, axis=-1)

# Kiểm tra có bao nhiêu pixel trắng
white_pixel_count = np.sum(mask_white)

# print("Số pixel màu trắng (255,255,255):", white_pixel_count)
...
# print("floorplan_fuse_map=",floorplan_fuse_map)
# print("legend=",legend)
...
#     """Làm mịn dữ liệu thô bằng phép đóng và mở"""
...
#     """Tìm các contour tường"""
#     contours, _ = cv2.findContours(binary_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
#     return contours

# def make_contour_axis_aligned(contour, epsilon=5):
#     """Xấp xỉ contour thành đoạn vuông góc (ngang/dọc)"""
...
#         dy = abs(p1[1] - p2[1])
#         if dx > dy:
#             p2[1] = p1[1]  # ép thành ngang
#         else:
#             p2[0] = p1[0]  # ép thành dọc

#     return np.array(aligned, dtype=np.int32).reshape(-1, 1, 2)

# def draw_clean_walls(shape, contours, epsilon=5):
#     """Vẽ lại các contour sau khi chỉnh vuông góc"""
...
# def straighten_wall_map(wall_map, epsilon=5):
#     """Pipeline hoàn chỉnh: mịn + vuông hóa tường"""
#     processed = preprocess_wall_map(wall_map)
#     contours = extract_contours(processed)
#     straightened = draw_clean_walls(processed.shape, contours, epsilon=epsilon)
#     return straightened
...
# def make_contour_axis_aligned(contour, epsilon=3, axis_tolerance=5):
#     """Xấp xỉ contour và làm thẳng các cạnh gần trục X/Y (cứng hơn)"""
#     approx = cv2.approxPolyDP(contour, epsilon, closed=True)
...
#             p2_aligned = [p1[0], p2[1]]  # thẳng trục Y
#         elif abs(dy) <= axis_tolerance:
#             p2_aligned = [p2[0], p1[1]]  # thẳng trục X
#         else:
#             p2_aligned = [p2[0], p2[1]]  # giữ nguyên nếu không gần trục

#         if not aligned or aligned[-1] != list(p1):
#             aligned.append(list(p1))
#         aligned.append(p2_aligned)
...
def smooth_map_skimage(grid, method='open', structure_size=3):
    selem = square(structure_size)
    if method == 'open':
        result = opening(grid, selem)
    elif method == 'close':
        result = closing(grid, selem)
    elif method == 'both':
        result = closing(opening(grid, selem), selem)
    else:
        raise ValueError("method phải là 'open', 'close' hoặc 'both'")
    return result.astype(int)
...
def smooth_edges_with_gaussian(arr, sigma=1):
    arr_float = arr.astype(float)
    blurred = gaussian_filter(arr_float, sigma=sigma)
    smoothed = (blurred > 0.5).astype(int)  # Ngưỡng lại
    return smoothed

def export_floorplan_json(newr, newcw, filename="floorplan.json"):
    # print("shape newr",newr.shape)
    # # print("shape newcw=",newcw.shape)
    # # print("newr",newr)
    # newr_2d = newr.squeeze(axis=2) 
    # # print("arr_2d",arr_2d)
    # newr = generic_filter(newr_2d, mode_filter_func, size=5, mode='nearest')
    # # print("smoothed",smoothed) 
    # newcw_2d = newr.squeeze(axis=2) 
    # # print("arr_2d",arr_2d)
    # newcw = generic_filter(newr_2d, mode_filter_func, size=5, mode='nearest')
    ...
    # straightened = smooth_map_skimage(arr_numpy, method='both', structure_size=3)
    # straightened = smooth_edges_with_gaussian(arr_numpy, sigma=2)
    # straightened = straighten_wall_map(arr_numpy, epsilon=1)
    # straightened = straighten_wall_map(straightened, epsilon=1)
      # Hiển thị kết quả
